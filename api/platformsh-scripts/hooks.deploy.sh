...
#   for your migration. It's meant to automate the minimum steps to have a working setup.
#
...
#   contain everything you need to run Gatsby and Drupal across environments.
#
...
# ########################################################################################################################
 ...
 if [ -f "$ENV_SETTINGS" ]; then
     ...
     if [ "$PROD_INSTALL" = false ]  || [ "$PREPPED_ENV" != "$PLATFORM_BRANCH" ]; then
         ...
         # c. Create Gatsby site consumer and configure previews.
