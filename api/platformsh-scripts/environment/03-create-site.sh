...
# NOTE:
#
# This script configures the Gatsby site associated with the current environment's OAuth consumer, and configures a
...
echo $UPDATED_DATA > $ENV_SETTINGS
CLIENT_ENVIRONMENT=$(echo $PLATFORM_ROUTES | base64 --decode | jq -r 'to_entries[] | select(.value.id == "client") | .key')
SITE_BASE_URL=${CLIENT_ENVIRONMENT%/}
