...
# NOTE:
#
# This script tracks settings that have changed related to the the Gatsby-Drupal connection for a particular
#   environment on Platform.sh. Throughout setup, plenty has been updated to ensure that for any given environment,
#   a unique Gatsby app container connects with its own unique Drupal container, and that this relationship is seamless
...
#       Rather than forcing the Gatsby container to build this file, Drupal is considered to be the source of truth and
...
PUBLIC_DRUPAL_BASE_URL=$(cat $ENV_SETTINGS | jq -r '.environment.site.url.base')
IMAGE_DOMAIN=$(cat $ENV_SETTINGS | jq -r '.environment.site.url.image_domain')
...
PUBLIC_DRUPAL_BASE_URL=$PUBLIC_DRUPAL_BASE_URL
...
export PUBLIC_DRUPAL_BASE_URL=$PUBLIC_DRUPAL_BASE_URL
export IMAGE_DOMAIN=$IMAGE_DOMAIN
